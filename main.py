...
BOT_TOKEN = "8933761060:AAGojwOXbaRFgXC1DhrkqizCvW2l-tWWv1k"
OWNER_ID  = 8700382637
...
class CPMNuker:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout, connector=aiohttp.TCPConnector(ssl=False)) as s:
                async with s.post(url, json=payload, headers=h) as r:
                    ...
                    # Keep HTTP status available to callers.  The old version
                    # discarded it whenever the server returned JSON, which
                    # made every HTTP error look like the generic RANK_FAILED.
                    if isinstance(data, dict):
                        data.setdefault("_http_status", status)
                        data.setdefault("_raw", text[:1000])
                    else:
                        data = {"data": data, "_http_status": status, "_raw": text[:1000]}
                    return data
        except Exception as e:
            log.error(f"HTTP: {e}")
            return {"_http_status": 0, "_error": str(e)}
    ...
    async def set_rank(self, uid):
        # Refresh the account/token first.  Do not silently ignore a failed load.
        ...
        r = await self._post(RANK_URL, {"data":json.dumps(rd, separators=(",", ":"))}, headers)

        # The endpoint may return a JSON object whose success flag is nested
        # under result/ok/success.  _ok() handles those forms.
        if r and self._ok(r):
            STORE["stats"]["total_unlocks"] = STORE["stats"].get("total_unlocks",0) + 1
            save_store(STORE)
            return {"ok":True}

        # Preserve the actual HTTP/API error instead of hiding it behind
        # RANK_FAILED.  This makes Railway logs and the Telegram message useful.
        status = r.get("_http_status", 0) if isinstance(r, dict) else 0
        raw = r.get("_raw", "") if isinstance(r, dict) else str(r)
        err = r.get("error") if isinstance(r, dict) else None
        detail = err if err else raw
        detail = str(detail).replace("\n", " ").strip()[:180]
        log.error(f"Rank failed: HTTP {status}; response={detail}")
        return {"ok":False,"message":f"RANK_FAILED [{status}]: {detail or 'empty response'}"}
